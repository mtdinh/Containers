...
var createCommand = cli.Command{
	...
	Action: func(context *cli.Context) error {
		...
		if config {
			...
			Type = context.Args().Get(1)
			if context.NArg() > 2 {
				return errors.Wrap(errdefs.ErrInvalidArgument, "with spec config file, only container id and type should be provided")
			}
		} else {
...
var listCommand = cli.Command{
	...
	Action: func(context *cli.Context) error {
		...
		fmt.Fprintln(w, "CONTAINER\tIMAGE\tRUNTIME\tTYPE\t")
		for _, c := range containers {
